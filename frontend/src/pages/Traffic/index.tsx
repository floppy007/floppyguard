...
function LineChart({ peers, mode, height = 160 }: ChartProps) {
	...
	// Find global max across all peers for this mode
	...
	// All samples share the same time window — use the union of timestamps
	// normalised to the last BW_HISTORY_SIZE (60) slots.
	const SLOTS = 60;

	const toPath = (peer: PeerBandwidth) => {
		if (!peer.history.length) return "";
		// Map slot index → value
		const points = peer.history.map((s, i) => {
	...
	// X axis ticks (time labels: "now", "-2.5m", "-5m", …)
	...
	return (
		...
		>
			...
			{yTicks.map(({ y, label }) => (
				<g key={label}>
					<line x1={PAD.left} y1={y} x2={W - PAD.right} y2={y} stroke="var(--tblr-border-color)" strokeWidth={1} strokeDasharray="3,3" />
			...
			<line
				...
				stroke="var(--tblr-border-color)" strokeWidth={1}
			/>
		</svg>
	);
}
				...
				<div key={peer.id} className="d-flex align-items-center gap-1">
					...
					{peer.history.length > 0 && (
						<span className="text-secondary" style={{ fontSize: "0.72rem" }}>
							↓ {rateFmt(peer.history[peer.history.length - 1]?.rx ?? 0)} ↑ {rateFmt(peer.history[peer.history.length - 1]?.tx ?? 0)}
						</span>
					)}
				</div>
			))}
...
const Traffic = () => {
	...
	// Current aggregate rate from latest bandwidth sample
	...
	// Limit chart to first 6 peers (most active)
			...
			<div className="row row-deck row-cards mb-3">
						...
						<div className="card-body">
							<div className="d-flex align-items-center gap-3">
								...
								<div>
									...
									<div className="fw-bold" style={{ fontSize: "1.1rem" }}>{rateFmt(currentRxRate)}</div>
									<div className="text-secondary" style={{ fontSize: "0.72rem" }}>{byteFmt(totalRx)} {intl.formatMessage({ id: "traffic.total" })}</div>
								</div>
							</div>
						</div>
					</div>
						...
						<div className="card-body">
							<div className="d-flex align-items-center gap-3">
								...
								<div>
									...
									<div className="fw-bold" style={{ fontSize: "1.1rem" }}>{rateFmt(currentTxRate)}</div>
									<div className="text-secondary" style={{ fontSize: "0.72rem" }}>{byteFmt(totalTx)} {intl.formatMessage({ id: "traffic.total" })}</div>
								</div>
							</div>
						</div>
					</div>
						...
						<div className="card-body">
							<div className="d-flex align-items-center gap-3">
								...
								<div>
									...
									<div className="fw-bold" style={{ fontSize: "1.1rem" }}>{activeLinks} / {links.length}</div>
								</div>
							</div>
						</div>
					</div>
				...
				<div className="col-sm-6 col-xl-3">
					<div className="card card-sm">
						<div className="card-body">
							<div className="d-flex align-items-center gap-3">
								<span className="bg-cyan text-white avatar"><IconWifi /></span>
								<div>
									<div className="text-secondary" style={{ fontSize: "0.78rem" }}>
										{intl.formatMessage({ id: "traffic.combined-rate" })}
									</div>
									<div className="fw-bold" style={{ fontSize: "1.1rem" }}>{rateFmt(currentRxRate + currentTxRate)}</div>
									<div className="text-secondary" style={{ fontSize: "0.72rem" }}>{bw.length} {intl.formatMessage({ id: "traffic.peers" })}</div>
								</div>
							</div>
						</div>
					</div>
				...
				<div className="col-lg-6">
					<div className="card h-100">
						...
						<div className="card-body pt-2">
							{bwLoading && <div className="text-secondary small py-4 text-center">{intl.formatMessage({ id: "traffic.warming-up" })}</div>}
							{!bwLoading && !hasHistory && (
								<div className="text-secondary small py-4 text-center">{intl.formatMessage({ id: "traffic.warming-up" })}</div>
							)}
							{!bwLoading && hasHistory && (
								<>
			...
			<div className="card">
				<div className="card-header d-flex align-items-center justify-content-between">
					...
					<a
						...
						onClick={(e) => { e.preventDefault(); navigate("/gateway"); }}
					>
				...
				<div className="table-responsive">
					<table className="table table-vcenter card-table">
						...
						<tbody>
							{links.length === 0 && (
								<tr>
									<td colSpan={6} className="text-secondary text-center small py-4">
								...
								.map((link) => {
									...
									return (
										<tr key={link.id}>
											<td>
												<div className="d-flex align-items-center gap-2">
													{isActive
														...
														: <IconCircle size={14} className="text-secondary flex-shrink-0" />}
													<div>
														<div className="fw-medium">{link.name || link.id}</div>
														<div className="text-secondary" style={{ fontSize: "0.75rem" }}>{link.interfaceName}</div>
													</div>
												</div>
											</td>
											<td>
											...
											<td className="text-secondary">{link.latestHandshake ? timeAgo(link.latestHandshake) : "—"}</td>
											<td>
												{isActive
													? <span className="badge bg-success-lt text-success">{intl.formatMessage({ id: "traffic.status-active" })}</span>
													: <span className="badge bg-secondary-lt text-secondary">{intl.formatMessage({ id: "traffic.status-idle" })}</span>}
											</td>
										</tr>
									);
								})}
